...
- (instancetype)fgl_removedEmojiString
{
    ...
    [self
        enumerateSubstringsInRange:NSMakeRange(0, [self length])
                           options:NSStringEnumerationByComposedCharacterSequences
                        usingBlock:^(NSString *substring, NSRange substringRange, NSRange enclosingRange, BOOL *stop) {
                            [buffer appendString:([substring fgl_isEmoji]) ? @"" : substring];
                        }];

    return buffer;
}

+ (void)fgl_initializeEmojiCheatCodes
{
...
- (NSString *)fgl_stringByReplacingEmojiCheatCodesWithUnicode
{
    if (!s_cheatCodesToUnicode) {
        [NSString fgl_initializeEmojiCheatCodes];
    }
    ...
    if ([self rangeOfString:@":"].location != NSNotFound) {
        ...
        [s_cheatCodesToUnicode enumerateKeysAndObjectsUsingBlock:^(NSString *key, id obj, BOOL *stop) {
            [newText replaceOccurrencesOfString:key
                                     withString:obj
